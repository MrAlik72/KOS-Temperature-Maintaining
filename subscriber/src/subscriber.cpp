...
constexpr auto Topic = "python/pid/temp"sv;
extern double desiredTemperature;
...
void Subscriber::on_message(const struct mosquitto_message *message)
{
    if (Topic == message->topic)
    {
        ...
        desiredTemperature = std::stod(std::string(payload));

        std::cout <<"[Subscriber] " << "Desired temperature updated to "
                    << desiredTemperature << std::endl;
#if 0
        if(!gpioInitflag) {
